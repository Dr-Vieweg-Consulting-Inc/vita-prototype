...
export const MaterialityUploader: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/api/stakeholders/${entityId}`)
      .then((res) => res.json())
      .then((data) => {
        setStakeholders(data);
        if (data.length > 0) setSelectedStakeholder(data[0].id);
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/materiality/${entityId}/${selectedStakeholder}`)
      .then((res) => res.json())
      .then((nestedData) => {
        const insideOutData = flattenMateriality(nestedData["inside-out"]);
        const outsideInData = flattenMateriality(nestedData["outside-in"]);

        console.log("insideOutData: =====> ", insideOutData);

        setDataInsideOut(insideOutData);
        setDataOutsideIn(outsideInData);
      })
      .catch((err) => console.error("Error fetching materiality data:", err));
  }, [entityId, selectedStakeholder]);
